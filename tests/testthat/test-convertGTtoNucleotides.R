test_that("convertGTtoNucleotides is able to construct a valid matrix for each sample", {
  chroms <- c("chr10", "chr19", "chr22", "chr1", "chr11", "chr7", "chr8", "chr13", "chr15", "chr12")
  ...
  df <- data.frame(
    chrom = c("chr19", "chr19", "chr19", "chr10", "chr10", "chr10", 
              "chr7", "chr7", "chr19", "chr13", "chr12", "chr15", 
              "chr8"
    ),
    gene = c("CYP2A13", "CYP2A6", "CYP2B6", "CYP2C19", "CYP2C8", "CYP2C9", 
             "CYP3A4", "CYP3A5", "CYP4F2", "NUDT15", "SLCO1B1", "CYP1A2", 
             "NAT2"
    )
  )
  ...
  for (i in 1:nrow(df)) {
    ...
    expect_equal(all(dim(orig) == dim(converted)), TRUE, label = "Converted matrix has same dimensions")
    expect_equal(all(grepl("[ACGT]", converted)), TRUE, label = "All positions have been converted to nucleotides")
  }
})
