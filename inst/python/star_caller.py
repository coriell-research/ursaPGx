...
def cyrius(d, genome, reference, threads):
    """CYP2D6 star caller to be used in R. 
    
    Parameters are passed from R instead of from command line arguments. 
    R coerces named lists to dictionaries and therefore this function takes a 
    dictionary of {sample_name : bam_filepath}
    The function returns a dictionary (json data from Cyrius).
    """
    # Use namedTuple instead of argument parser Namespace
    Parameters = namedtuple("Parameters", "genome threads countFilePath reference")
    parameters = Parameters(genome=genome, threads=threads, 
                            countFilePath=None, reference=reference)
    ...
    for k, v in d.items():
        index_name = None
        if '##idx##' in v:
            v, index_name = v.split('##idx##')

        count_file = None
        cyp2d6_call = d6_star_caller(v, call_parameters, threads, 
                                     count_file, reference_fasta=reference, 
                                     index_name=index_name)._asdict()
        final_output.setdefault(k, cyp2d6_call)
